...
// Destructured to only the icons used in DEFAULT_IMAGES
const {
    CPP,
    CSS,
    GithubLight: Github,
...
// Full skill set matching Vaanya's resume
...
export default function DomeGallery({
    ...
    // ── AUTO-SPIN + HOVER TOOLTIP ──────────────────────────────────────────
    const autoSpinRAF = useRef<number | null>(null);
    const isHoveredRef = useRef(false);
    const [hoveredAlt, setHoveredAlt] = useState<string | null>(null);
    const [tooltipPos, setTooltipPos] = useState({ x: 0, y: 0 });

    // Start the auto-spin loop on mount; it will pause when isHoveredRef is true
    useEffect(() => {
        const SPEED = 0.08; // degrees per frame
        const step = () => {
            if (!isHoveredRef.current && !draggingRef.current && !inertiaRAF.current) {
        ...
        <>
            ...
            >
                <main
                    ...
                    onMouseLeave={() => { isHoveredRef.current = false; setHoveredAlt(null); }}
                    style={{
                        touchAction: "none",
                ...
                {hoveredAlt && (
                    <div
                        className="pointer-events-none absolute z-50 px-3 py-1 rounded-full text-xs font-medium shadow-lg"
                        style={{
                            left: tooltipPos.x,
                            top: tooltipPos.y,
