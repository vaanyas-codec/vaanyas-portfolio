...
import { ExternalLink, Code, Heart, ChevronLeft, ChevronRight, ShieldAlert, Microscope, ScanFace } from 'lucide-react';
  ...
  const projects = [
    {
      ...
      description: "Leakage-free ML pipeline on a severely imbalanced dataset (0.17% fraud rate) using SMOTE, feature scaling, and outlier capping. XGBoost achieved 91.23% F1 and 99.31% ROC-AUC across 5-fold CV. Manuscript under review at the Journal of Young Investigators.",
      technologies: ["Python", "scikit-learn", "XGBoost", "Pandas", "SMOTE"],
      icon: '',
      emoji: 'ShieldAlert',
      detailsUrl: "/projects/credit-card-fraud-detection",
      githubUrl: socialLinks.repositories.projectOne
    },
    {
      title: "Skin Lesion Classifier",
      description: "EfficientNet-B0 transfer learning on HAM10000 (10K images, 58:1 class imbalance) for 7-class skin lesion classification. Switched to focal loss to raise melanoma recall from 60% to 72%; Grad-CAM confirms the model attends to the lesion, not background.",
      technologies: ["PyTorch", "EfficientNet-B0", "FastAPI", "React", "Grad-CAM"],
      icon: '',
      emoji: 'Microscope',
      detailsUrl: "/projects/skin-lesion-classifier",
      githubUrl: socialLinks.repositories.projectTwo
    },
    {
      title: "Real-Time ASL Detection",
      description: "Real-time ASL recognizer (webcam → MediaPipe landmarks → sklearn classifier), built after a WLASL Transformer and MediaPipe Model Maker attempt fell short. Diagnosed a live-accuracy collapse to insufficient training-clip variety and reached 94.5% cross-validated accuracy across 28 words.",
      technologies: ["Python", "OpenCV", "MediaPipe", "scikit-learn", "Claude API"],
      icon: '',
      emoji: 'ScanFace',
      detailsUrl: "/projects/realtime-asl-detection",
                ...
                <CardHeader>
                  <div className="flex items-start gap-3">
                    <div className="w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0"
                      style={{ background: themeColors.interactive.primary }}>
                      {(project as typeof project & { emoji?: string }).emoji === 'ShieldAlert' && <ShieldAlert className="w-6 h-6" style={{ color: themeColors.text.accent }} />}
                      {(project as typeof project & { emoji?: string }).emoji === 'Microscope' && <Microscope className="w-6 h-6" style={{ color: themeColors.text.accent }} />}
                      {(project as typeof project & { emoji?: string }).emoji === 'ScanFace' && <ScanFace className="w-6 h-6" style={{ color: themeColors.text.accent }} />}
                    </div>
